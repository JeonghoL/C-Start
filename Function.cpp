...
함수 인자 ; 해당 함수를 사용할 때 넘겨줄 값이 있을 경우 인자를 활용한다. 인자는 
있을수도 있고 없을 수도 있다.
...
int Sum(int a, int b) {
	// return 키워드를 이용해서 이 함수이ㅡ 반환타입에 맞는 값을 반환해야 한다.
	return a + b;
}
...
void OutputText() {
	std::cout << "OutputText Function!" << std::endl;
}
...
// 디폴트 인자는 가장 오른쪽부터 지정해 주어야 한다.
void Output(int iNum1, int iNum2 = 10) {
	std::cout << iNum1 << std::endl;
	std::cout << iNum2 << std::endl;
}

// 아래와 같은 경우 함수의 디폴트 인자이자 Output 함수의 오버로딩인ㄱ데, 인자를 10 하나
// 넣어주면 위의 함수와 아래 함수 중 어떤 함수를 호출하여 사용해야 할지 컴파일러가 결정하지
// 못한다. 그러므로 이러한 부분을 잘 고려하여 사용해야 한다.
...
// 아래 함수는 결과값을 인자로 받아와서 결과를 반환해주었다. 이러한 형태의 함수를
